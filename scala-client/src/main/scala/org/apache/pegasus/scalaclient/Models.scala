...
case class PegasusResult(bytes: Array[Byte]) {
  def as[V](implicit ser: Serializer[V]): V = ser.deserialize(bytes)
  def asOpt[V](implicit ser: Serializer[V]): Option[V] =
    if (isNull) None else Some(ser.deserialize(bytes))
  def isNull = bytes == null
}

case class PegasusResultList(list: List[Array[Byte]]) {
  def as[V](implicit ser: Serializer[V]): List[V] = list.map(ser.deserialize)
  def asOpt[V](implicit ser: Serializer[V]): List[Option[V]] = list.map { v =>
